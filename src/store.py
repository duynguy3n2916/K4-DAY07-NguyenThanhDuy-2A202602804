...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        ...
        if self._store and len(embedding) != len(self._store[0]["embedding"]):
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        if top_k <= 0 or not records:
            return []
        vector = list(self._embedding_fn(query))
        results = []
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        Each input is already a chunk. Preserve its parent doc_id in metadata.
        """
        for doc in docs:
    ...
    def search_with_filter(self, query: str, top_k: int = 3, metadata_filter: dict = None) -> list[dict]:
        ...
        records = [r for r in self._store if all(
            key in r["metadata"] and r["metadata"][key] == value
